...
# CSV 파일 경로 설정
result_csv_path = '/userHome/userhome2/hyejin/paper_implementation/res/RMSE/17_bank_ensemble_method_res.csv'
result_imputation_path = '/userHome/userhome2/hyejin/paper_implementation/res/imputation_res/17_bank_impute_res'
...
def main(args):
    ...
    for i  in range(30):
        ...
        y_trnval_knn = pd.DataFrame(y_trnval, columns=['y'])
        x_tst_knn = pd.DataFrame(x_tst, columns=train_col)
        y_tst_knn = pd.DataFrame(y_tst, columns=['y'])
        ...
        y_trnval_datawig = pd.DataFrame(y_trnval, columns=['y'])
        x_tst_datawig = pd.DataFrame(x_tst, columns=train_col)
        y_tst_datawig = pd.DataFrame(y_tst, columns=['y'])
