...
for iteration in range(num_iterations):
    ...
    # 학습을 위한 데이터 준비
    train_X_knn_imputed = train_data_knn_imputed.drop(columns=['y'])
    train_y_knn_imputed = train_data_knn_imputed['y']
    test_X_knn_imputed = test_data_knn_imputed.drop(columns=['y'])
    test_y_knn_imputed = test_data_knn_imputed['y']

    train_X_zero_imputed = train_data_zero_imputed.drop(columns=['y'])
    train_y_zero_imputed = train_data_zero_imputed['y']
    test_X_zero_imputed = test_data_zero_imputed.drop(columns=['y'])
    test_y_zero_imputed = test_data_zero_imputed['y']
    ...
    # accuracy 계산
    ensemble_accuracy = accuracy_score(test_data['y'].values, np.round(avg_predictions))
