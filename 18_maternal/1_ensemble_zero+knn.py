...
# CSV 파일 경로 설정
result_csv_path = '/userHome/userhome2/hyejin/paper_implementation/res/18_maternal_ensemble_method_res.csv'
...
for iteration in range(num_iterations):
    ...
    # 학습을 위한 데이터 준비
    train_X_knn_imputed = train_data_knn_imputed.drop(columns=['RiskLevel'])
    train_y_knn_imputed = train_data_knn_imputed['RiskLevel']
    test_X_knn_imputed = test_data_knn_imputed.drop(columns=['RiskLevel'])
    test_y_knn_imputed = test_data_knn_imputed['RiskLevel']

    train_X_zero_imputed = train_data_zero_imputed.drop(columns=['RiskLevel'])
    train_y_zero_imputed = train_data_zero_imputed['RiskLevel']
    test_X_zero_imputed = test_data_zero_imputed.drop(columns=['RiskLevel'])
    test_y_zero_imputed = test_data_zero_imputed['RiskLevel']
...
model_knn_imputation.sess.close()
model_zero_imputation.sess.close()
