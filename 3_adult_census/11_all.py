...
def main(args):
   ...
   # 레이블 인코딩 적용
    ...
    for i  in range(10):
        ...
        y_trnval_zero = pd.DataFrame(y_trnval, columns=['target'])
        x_tst_zero = pd.DataFrame(x_tst, columns=train_col)
        y_tst_zero = pd.DataFrame(y_tst, columns=['target'])
        ...
        prediction_model_4 = model_zero_imputation.sess.run(model_zero_imputation.pred, feed_dict={model_zero_imputation.x: test_X_zero_imputed.values})
        combined_predictions = (prediction_model_1 + prediction_model_2 + prediction_model_3 + prediction_model_4) / 4
        ...
        print(str(i+1)+"th knn rmse === : ", np.mean(zero_rmse))
