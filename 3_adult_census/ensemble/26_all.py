...
# CSV 파일 경로 설정
result_csv_path = '/userHome/userhome2/hyejin/paper_implementation/res/RMSE/3_adult_ensemble_method_res.csv'
result_imputation_path = '/userHome/userhome2/hyejin/paper_implementation/res/imputation_res/3_adult_impute_res'
...
def main(args):
    ...
    for i  in range(30):
        ...
        y_trnval_knn = pd.DataFrame(y_trnval, columns=['target'])
        x_tst_knn = pd.DataFrame(x_tst, columns=train_col)
        y_tst_knn = pd.DataFrame(y_tst, columns=['target'])
        ...
        y_trnval_datawig = pd.DataFrame(y_trnval, columns=['target'])
        x_tst_datawig = pd.DataFrame(x_tst, columns=train_col)
        y_tst_datawig = pd.DataFrame(y_tst, columns=['target'])
