...
################################################################################
# 데이터를 불러오기
missing_path = '00_dataset/missing/23_ai4i.csv'
missing_df = pd.read_csv(missing_path)

# 불필요한 열 제거 및 컬럼명 변경
missing_df = missing_df.drop(['class_1','class_2'], axis=1)
missing_df = missing_df.rename(columns={'class_0': 'class'})
...
################################################################################
preprocessing_path = '00_dataset/preprocessing/23_ai4i.csv'

preprocessing_df = pd.read_csv(preprocessing_path)

# 불필요한 열 제거 및 컬럼명 변경
preprocessing_df =preprocessing_df.drop(['class_1','class_2'], axis=1)
preprocessing_df = preprocessing_df.rename(columns={'class_0': 'class'})
