...
for iteration in range(num_iterations):
    ...
    for col in train_col:
        ...
        if col not in imputers:  # 이 부분이 추가됨
            ...
            print(" == else imputer === ", imputer)
        print("== else imputer_fit == ", imputer)
        print("== else imputers[col] == ", imputers[col])

    # Impute missing values for each column in train_data
    train_imputed_data = {}
    print("=== imputers.items ==== ",imputers.items())
    for col, imputer in imputers.items():
        print("=== col ==== ",col)
        print("=== imputer ==== ",imputers)
        predictions = imputer.predict(train_data)
        print("=== train_data_prdictions ==== ",predictions)
        train_imputed_data[col] = predictions[col + '_imputed']  # '_imputed' is added by datawig
        print("=== train_imputed_data[col] ==== ",len(train_imputed_data[col]))
