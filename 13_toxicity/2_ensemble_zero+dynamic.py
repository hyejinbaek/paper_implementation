...
# CSV 파일 경로 설정
result_csv_path = '/userHome/userhome2/hyejin/paper_implementation/res/13_toxicity_ensemble_method_res.csv'
...
class DynamicImputationModel:
    ...
    def build_model(self, x):
        logging.warning("====== build model log")
        for _ in range(self.num_layers):
            x = tf.layers.dense(x, self.num_hidden, activation=tf.nn.tanh)
        logging.warning("====== build model 222 log")    
