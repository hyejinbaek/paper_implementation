...
# CSV 파일 경로 설정
result_csv_path = '/userHome/userhome2/hyejin/paper_implementation/10_lymphography_ensemble_method_res.csv'
...
for iteration in range(num_iterations):
    ...
    print(" === train_data_knn_imputed === ", train_data_knn_imputed)
    print(" === test_data_knn_imputed === ", test_data_knn_imputed)

    # 데이터 결측치 채우기 (Zero Imputation)
    train_data_zero_imputed = train_data.fillna(0)
    test_data_zero_imputed = test_data.fillna(0)
    print(" === train_data_zero_imputed === ", train_data_zero_imputed)
    print(" === test_data_zero_imputed === ", test_data_zero_imputed)
