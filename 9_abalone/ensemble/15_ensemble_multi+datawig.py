...
for iteration in range(num_iterations):
    ...
    # 예측값 평균 계산
    avg_predictions = (datawig_imputed_model + multi_imputed_model) / 2
    ...
    result_df = pd.DataFrame({
            'multi_model_imputation': multi_imputed_model.flatten(),
            'datawig_imputation': datawig_imputed_model.flatten()
        })
