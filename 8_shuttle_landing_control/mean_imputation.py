...
# 데이터 파일 경로 설정
data_pth = './shuttle-landing-control.data'

# 데이터 불러오기
df_data = pd.read_csv(data_pth)
col_data = df_data.columns = ['class','STABILITY', 'ERROR', 'SIGN', 'WIND', 'MAGNITUDE', 'VISIBILITY']
train_col =['STABILITY', 'ERROR', 'SIGN', 'WIND', 'MAGNITUDE', 'VISIBILITY']

df_data['STABILITY'] = df_data['STABILITY'].replace('*',0).astype(int)
df_data['ERROR'] = df_data['ERROR'].replace('*',0).astype(int)
df_data['SIGN'] = df_data['SIGN'].replace('*',0).astype(int)
df_data['WIND'] = df_data['WIND'].replace('*',0).astype(int)
df_data['MAGNITUDE'] = df_data['MAGNITUDE'].replace('*',0).astype(int)
data = df_data

# 범주형 피처 선택
categorical_columns = train_col
