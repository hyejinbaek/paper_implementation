...
#     def shuffle_train_model(self, train_data, train_labels, num_epochs, batch_size, optimizer):
#         # 초기화
#         self.build_model()  # 모델을 생성하는 메서드 (이미 있다면 불필요)
...
def main(args):
    ...
    for i  in range(2):
        ...
        print(" === train_X_ensemble ===", train_X_ensemble)
        print(" === train_X_ensemble ===", train_X_ensemble.size)
        ...
        print(" === train_y_ensemble ===", train_y_ensemble)
        print(" === train_y_ensemble ===", train_y_ensemble.size)
        ...
        num_classes = 2  # binary classification for breast cancer dataset
        model_scale = 1.0
        shuffle_group = 2
        # model_ensemble = ShuffleNetV2()
        model_ensemble = ShuffleNetV2(num_classes=2, shuffle_group=2, model_scale=1.0)
        
        model_ensemble.shuffle_train_model(train_X_ensemble, train_y_ensemble)

        # 앙상블 모델로 훈련
        model_ensemble.fit(train_X_ensemble, train_y_ensemble, epochs=50, batch_size=32)
        ...
        accuracy_net = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(model_ensemble(test_X_ensemble, training=False), axis=1), test_y_ensemble), tf.float32)).numpy()
        print("==========================================")
        print(f'Accuracy Net on Test Set: {accuracy_net}')
        print("==========================================")
