...
def main(args):
    ...
    for i  in range(2):
        ...
        print(" === test_X_knn_imputed === ", test_X_knn_imputed.shape)
        ...
        print(" === knn_imputed_model === ", knn_imputed_model.shape)
        print(" === dynamic_imputed_model === ", dynamic_imputed_model.shape)

        ensemblemodel = ShuffleModel()
        # 모델 요약 출력
        ensemblemodel.build((None, 57, 1))
        print("=== ensemble model ====", ensemblemodel.summary())
        ensemblemodel.compile(optimizer='adam', loss=ensemblemodel.loss, metrics=[ensemblemodel.accuracy])
        
        ensemblemodel.fit([knn_imputed_model, dynamic_imputed_model], epochs=10, batch_size=32)
        
        
        imputed_results = ensemblemodel.predict([knn_imputed_model, dynamic_imputed_model])
        print("=-=== imputed_results === ",imputed_results)
        
        
        # knn_imputed_model = pd.DataFrame(knn_imputed_model, columns=['Class'])
        # dynamic_imputed_model = pd.DataFrame(dynamic_imputed_model, columns=['Class'])

        # ensemble_shuffle = ShuffleModel(num_layers=3, num_hidden=128, dim_y=1, num_features=len(dynamic_imputed_model))
        # print(ensemble_shuffle)
        # ensemble_shuffle.train_model(knn_imputed_model, dynamic_imputed_model, num_epochs=50, batch_size=32)
        # ensemble_imputation = ensemble_shuffle.get_accuracy(knn_imputed_model, dynamic_imputed_model)
        
        # print("==========================================")
        # print(str(i+1)+"th Ensemeble accuracy === : ", ensemble_imputation)
        # print("==========================================")
        # # # 예측값 평균 계산
        # # avg_predictions = (knn_imputed_model + dynamic_imputed_model) / 2

        # # accuracy 계산
        # ensemble_accuracy = accuracy_score(y_tst, np.round(ensemble_imputation))
        ...
        # RMSE 계산
        # rmse = sqrt(((original_y_test.values - ensemble_imputation.flatten()) ** 2).mean())
