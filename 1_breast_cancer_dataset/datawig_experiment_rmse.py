...
#print("? ==== dtype", data_with_missing.dtypes)
...
for iteration in range(num_iterations):
    ...
    train_y = train_data['Class']  # Keep it as a DataFrame
    print(" ==== train_y ====", train_y)
    test_X = test_imputed_df[train_col]  # Select only the columns for testing
    print(" ==== test_X ====", test_X)
    test_y = test_data['Class']  # Keep it as a DataFrame
    print(" ==== test_y ====", test_y)
    ...
    # RMSE 계산 및 저장
    y_pred = model.sess.run(model.pred, feed_dict={model.x: test_X.values})
    rmse = calculate_rmse(test_y.values, y_pred)
