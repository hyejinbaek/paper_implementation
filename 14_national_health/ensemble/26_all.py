...
# 실행 명령어 : python 26_all.py --seed 0 --missing_rate 20 --num_mi 5 --m 10 --tau 0.05
import os
...
# CSV 파일 경로 설정
result_csv_path = '/userHome/userhome2/hyejin/paper_implementation/res/RMSE/14_national_ensemble_method_res.csv'
result_imputation_path = '/userHome/userhome2/hyejin/paper_implementation/res/imputation_res/14_national_impute_res'

# 결과를 저장할 리스트 초기화
results = []
model_imputation_res_list =[]
...
def main(args):
    ...
    prepro_y = prepro_data['age_group']
    ...
    for i  in range(30):
        ...
        y_trnval_knn = pd.DataFrame(y_trnval, columns=['age_group'])
        x_tst_knn = pd.DataFrame(x_tst, columns=train_col)
        y_tst_knn = pd.DataFrame(y_tst, columns=['age_group'])
        ...
        y_trnval_datawig = pd.DataFrame(y_trnval, columns=['age_group'])
        x_tst_datawig = pd.DataFrame(x_tst, columns=train_col)
        y_tst_datawig = pd.DataFrame(y_tst, columns=['age_group'])
        ...
        model_imputation_res = {
            'Dataset' : '14_national',
            'Experiment': i + 1,
            'multi_model' : multi_model.flatten(),
            'datawig_imputed_model' : datawig_imputed_model.flatten(),
            'dynamic_imputed_model' : dynamic_imputed_model.flatten(),
            'knn_imputed_model' : knn_imputed_model.flatten(),
            'zero_imputed_model' : zero_imputed_model.flatten()
        }
        model_imputation_res_list.append(model_imputation_res)
        
        # 예측값 평균 계산
        avg_predictions = (multi_model + datawig_imputed_model + dynamic_imputed_model + knn_imputed_model + zero_imputed_model) / 5
        ...
        result = {
            'Dataset' : '14_national',
            'method' : '26_all(multi+dynamic+zero+knn+datawig)',
            'Experiment': i + 1,
    ...
    # imputation 결과 CSV 파일에 추가로 저장
    results_df = pd.DataFrame(model_imputation_res_list)
    if os.path.exists(result_imputation_path):
        results_df.to_csv(result_imputation_path, mode='a', header=False, index=False)
    else:
        results_df.to_csv(result_imputation_path, index=False)

    print("Results saved to:", result_imputation_path)
